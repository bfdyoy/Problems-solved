...
int main()
{
    unsigned int no_max_vertices = 10;
    ...
    for (int tests_iterator = 1; tests_iterator <= TESTS_NO; tests_iterator++)
    {
        ...
        // and initialize it to 0, since the colors start from 1
        colors = (int*) calloc(no_vertices+1, sizeof(int));

        // The maximum number of colors can be the number of vertices
        // for a complete graph
        no_colors = no_vertices;
        ...
        graph_coloring(grph, no_colors, colors, 1);
        minimum_colors = get_max(colors, no_colors);
        ...
        fprintf(test_out, "%d", minimum_colors);
